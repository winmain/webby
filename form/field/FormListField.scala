...
class FormListField[F <: Form](val id: String, var factory: () => F)
  extends Field[Vector[F]] {
  self =>
   ...
   */
  def template(tag: String = "p", cls: String = null, mobileMulti: Boolean = false)(body: F => Any)(implicit view: HtmlView): HtmlBase =
    view.tag(tag).id(id + "-template").cls("hide").cls(cls).clsIf(mobileMulti, "mobile-multi") < body(formStub)
  def blockTemplate(tag: String = "section", cls: String = "form-block subform")(body: F => Any)(implicit view: HtmlView): HtmlBase =
    view.tag(tag).id(id + "-template").cls("hide").cls(cls) {blockRemoveTag; body(formStub)}

  def listPlaceholder(implicit view: HtmlView): CommonTag = view.div.id(id + "-list")
  def smallAddTag(implicit view: HtmlView): CommonTag = view.a.hrefAnchor.cls("dotted").id(id + "-add")
  def smallRemoveTag(implicit view: HtmlView): CommonTag = view.a.hrefAnchor.cls("small-remove").id(removeId)
  def blockAddTag(implicit view: HtmlView): CommonTag = view.a.hrefAnchor.cls("block-add").id(id + "-add")
  def blockRemoveTag(implicit view: HtmlView): CommonTag = view.a.hrefAnchor.cls("block-remove").id(removeId).title("Удалить блок")
