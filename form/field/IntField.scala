...
import webby.commons.text.html.{HtmlBase, StdInputTag}
...
class BaseIntField(val id: String) extends ValueField[Int] with PlaceholderField[Int] {self =>
  ...
  def inputNumber(implicit view: HtmlView): StdInputTag = placeholderInput(view.inputTel)
}
...
class PagerField(var step: Int, var nearRadius: Int, val id: String = "page") extends ValueField[Int] {self =>
  ...
  def pagerComponent()(implicit view: HtmlView): HtmlBase = pagerComponent(get * step + 1)

  def pagerComponent(totalCount: Int)(implicit view: HtmlView): HtmlBase = {
    ???
