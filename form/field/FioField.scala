 ...
 */
class FioField(val id: String) extends ValueField[Fio] with PlaceholderField[Fio] {self =>
  ...
  override def parseJsValue(node: JsonNode): Either[String, Fio] = {
    if (node == null || node.isNull || node.size() == 0) Right(nullValue)
      ...
      def trimmedCapString(node: JsonNode): String = {
        ...
        if (text == null) null
          ...
          val s = text.trim
          if (s.isEmpty) null else s.unescapeAndCleanHtmlEntities.capitalize
        }
      }
      ...
      if (family == null || name == null) Left("Некорректное значение")
        ...
        Right(Fio(family, name, patronymic = if (node.size() == 3) trimmedCapString(node.get(2)) else null))
    }
  }
  override def toJsValue(v: Fio): AnyRef = if (v == null) null else Seq(v.family, v.name, v.patronymic)

  override def nullValue: Fio = null
  ...
  override def validateFieldOnly: ValidationResult = {
    if (!FioField.allowedChars.matchesAllOf(get.family))
      Invalid("Введите фамилию русскими буквами")
    else if (!FioField.allowedChars.matchesAllOf(get.name))
      Invalid("Введите имя русскими буквами")
    else if (get.patronymic != null && !FioField.allowedChars.matchesAllOf(get.patronymic))
...
case class Fio(family: String, name: String, @Nullable patronymic: String)
