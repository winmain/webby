package lib.form
import lib.form.field.{DbConnector, Field}
import querio._
import webby.api.mvc.ResultException

import scala.collection.mutable

trait FormWithDb[TR <: TableRecord, MTR <: MutableTableRecord[TR]] extends Form with FormDbConnectors[TR, MTR] {
  ...
  def load(id: Int)(implicit conn: Conn): Boolean = {
    db.sql(_.findById(table, id)) match {
  ...
  def innerSave(gotChangedItem: Option[(ChangedItemType, Int)])(implicit dt: DataTr): MTR = {
    ...
    val body = {() =>
      ...
      val id: Int = original match {
        case Some(o) =>
          db.updateChanged(o, record)
          record._primaryKey
        case None =>
          db.insert(record).getOrElse(sys.error("Form must have primary key field (id)"))
      }
      ...
      for ((tpe, itemId) <- changedItem; orig <- original) {
        ...
        for (field <- table._fields if field.get(orig) != field.getM(record) && !ignoreFields.contains(field))
          ChangedFieldsDao.insert(tpe, itemId, field, id)
      }
