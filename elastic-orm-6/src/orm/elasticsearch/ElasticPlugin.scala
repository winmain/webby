...
class ElasticPlugin(app: Application) extends Plugin {
  ...
  val transportAddresses: Vector[TransportAddress] = {
    val result = Vector.newBuilder[TransportAddress]
    app.configuration.getStringList("elastic.transportAddress")
      ...
      .forEach {line =>
        ...
        result += new TransportAddress(InetAddress.getByName(host), port)
      }
    result.result()
  }
