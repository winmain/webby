...
abstract class BaseQuartzPlugin(val testMode: Boolean) extends Plugin {
  ...
  /**
    ...
    */
  protected def jsMapper: ObjectMapper = StdJs.js.mapper

  /**
    * Фабрика для создания логов крона [[CronLog]]. Если не переопределена, то логи не сохраняются.
    */
  protected def cronLogFactory: CronLogFactory = StubCronLogFactory

  protected def createCronLog(group: String, name: String): CronLog = cronLogFactory.forQuartz(group, name)
