...
object ElasticSearch {
  ...
  private lazy val elasticSearch = new AppPluginHolder[ElasticPluginConf]()
  def client: Client = elasticSearch.get.client
  def clusterName = elasticSearch.get.clusterName
  ...
  def httpRequest(method: HttpMethod, path: String, data: Option[Array[Byte]] = None): String = {
    val es: ElasticPluginConf = elasticSearch.get
    val conn = new URL("http", es.httpHost.getHostAddress, es.httpPort, path).openConnection().asInstanceOf[HttpURLConnection]
